	...
	*/
	function workAndWeekendDaysInThisMonth() {
		...
		for ($day=0; $day < $daysInTheMonth; $day++) {
			...
			//Если это не выходной день, то добавляем к счётчику будних дней единицу
			if (!in_array($dayOfWeek, $weekEndDays)) {
				$numberOfWorkDaysInCurrentMonth++;
			}

			//Если текущий день совпадает с днём в текущей итерации, то вычисляем порядковый номер рабочего дня в этом месяце
